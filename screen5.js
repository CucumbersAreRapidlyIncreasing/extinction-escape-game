(() => {
  ...
  let lastFocus = null;
  let replyIndex = 0;
  const replies = ["なんのことでしょう？", "私にはわかりません。", "エンジン起動手順を確認してみてください。"];

  function lockPage(locked) { body.classList.toggle("modal-open", locked); }
  ...
  function appendMessage(role, text) {
    const wrapper = document.createElement("div"); wrapper.className = `chat-message chat-message-${role}`;
  ...
  document.addEventListener("keydown", event => {
    ...
    if (!lightbox.hidden) closeLightbox(); else if (!inventoryLayer.hidden) closeInventory(); else if (!chatPanel.hidden) closeChat();
  });
})();
