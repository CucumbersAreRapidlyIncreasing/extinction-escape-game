(() => {
  ...
  let completed=Boolean(window.GameProgress?.getState().screen7.palmScanCompleted);
  let cipherTableReceived=Boolean(window.GameProgress?.getState().screen6.cipherTableReceived);
  ...
  function showComplete({scroll=false}={}){completed=true;terminal.classList.remove("is-scanning");terminal.classList.add("is-complete");status.textContent="SCAN COMPLETE";message.textContent="パイロット認証を確認しました。";completePanel.hidden=false;window.GameProgress?.updateScreen7({palmScanCompleted:true});if(scroll)completePanel.scrollIntoView({behavior:"smooth",block:"center"})}
